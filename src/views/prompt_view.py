...
class PromptView:
    ...
    def render_creation_form(self):
        ...
        with st.form("prompt_form"):
            ...
            # 평가 섹션
            self._render_evaluation_fields()
    ...
    def _handle_submit(self):
        ...
        try:
            ...
            self.manager.create_prompt(data)
            st.success("프롬프트가 성공적으로 저장되었습니다!")
            
            if self.evaluation_fields['auto_increment']:
                new_version = self.manager.increment_version(self.basic_info['version'])
    ...
    def _validate_form(self) -> bool:
        """폼 유효성 검사"""
        if not self.basic_info['title']:
            st.error("제목을 입력해주세요.")
            return False
            
        if not self.basic_info['version']:
            st.error("버전을 입력해주세요.")
            return False
            
        if not self.manager.validate_version(self.basic_info['version']):
            st.error("올바른 버전 형식이 아닙니다. (예: 1.0.0)")
            return False
            
        if not self.content_fields['prompt_content']:
